...
helper.sendGetQSKeyRequest = function (uri, qs, keys, keyType, getAllPages, cb, pagesData) {
  ...
  request(options, function(error, response, body) {
    if (getAllPages) {
      ...
      if (nextUri != null) {
        helper.sendGetQSKeyRequest(nextUri, qs, keys, keyType, getAllPages, cb, pagesData);
      } else {
...
helper.sendGetQSRequest = function (uri, qs, keys, cb) {
  helper.sendGetQSKeyRequest(uri, qs, keys, 'restKey', true, cb);
}

// Simple version to use with null query string (assumes restKey and getAllPages=true)
helper.sendGetRequest = function (uri, keys, cb) {
  helper.sendGetQSKeyRequest(uri, null, keys, 'restKey', true, cb);
}

// Simple version to use with null query string (assumes getAllPages=true)
helper.sendGetKeyRequest = function (uri, keys, keyType, cb) {
  helper.sendGetQSKeyRequest(uri, null, keys, keyType, true, cb);
}

// Send the PUT or POST with a query string
helper.sendPutOrPostQS = function(uri, method, qs, keys, keyType, cb) {
  // This gets either the restKey or adminKey
  var localKey = keys[keyType]
...
helper.sendPutOrPostBody = function(uri, method, body, keys, keyType, cb) {
  // This gets either the restKey or adminKey
  var localKey = keys[keyType]
...
helper.sendDelete = function(uri, keys, keyType, cb) {
  // This gets either the restKey or adminKey
  var localKey = keys[keytype]
