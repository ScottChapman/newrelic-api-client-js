...
alerts.infrastructureConditions.list = function(policyId, configId, cb) {
  var url = urls.api.infrastructureAlertsConditions.list;
  var qs = { 'policy_id': policyId }
...
alerts.NRQLConditions.create = function(configId, policyId, cond, cb) {
  ...
  var qs = { 'policy_id': policyId }
  helper.sendPutOrPostQS(url, 'POST', qs, configId, 'adminKey', cb);
}
...
alerts.conditions.create = function(configId, policyId, cond, cb) {
  ...
  helper.sendPutOrPost(url, 'POST', cond, configId, 'adminKey', cb);
}

alerts.conditions.update = function(configId, policyId, cond, cb) {
  var url = urls.api.alertsConditions.updateDelete.replace('{policy_id}',policyId)
  helper.sendPutOrPost(url, 'PUT', cond, configId, 'adminKey', cb);
}
